...
from aiuw import settings

class AiuwPipeline(object):
    def process_item(self, item, spider):
        return item

class AiuwImagesPipe(ImagesPipeline):
...
class InsertTitlePipe(object):
    ...
    def insertTitleSql(self, tx,item):
        ...
        sql = "insert into aiuw"
        sql_insert = sql + "(site,title,tag,origin_url,new_url,mb,pixel,size,format) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        
        tx.execute(sql_insert,(item['site'],item['title'], 
