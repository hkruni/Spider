...
from qijia import settings

class QijiaPipeline(object):
    def process_item(self, item, spider):
        return item

class QijiaImagesPipe(ImagesPipeline):
...
class InsertTitlePipe(object):
    ...
    def insertTitleSql(self, tx,item):
        ...
        sql = "insert into qijia"
        sql_insert = sql + "(site,title,keywords,tag,origin_url,new_url,mb,pixel,size,format) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        
        tx.execute(sql_insert,(item['site'],item['title'],item['key'], 
                               item['tag'],item['origin_url'],item['new_url'],item['mb'],item['pixel'],item['size'],item['format']))
